...
class RegisterForm(forms.Form):
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control'}), label='password', validators=[
        ...
        RegexValidator(
            '[!@#$%^&_]+', 'The password must contain at least one special symbol "!@#$%^&_"'),
        RegexValidator('^.{5,20}$', 'The password must be 5-15 characters long')], required=True)
...
class LoginForm(forms.Form):
    email = forms.CharField(label='Email', validators=[EmailValidator(
        'Please enter a valid e-mail')], widget=forms.TextInput(attrs={'class': "form-control", "type": "email"}), required=True)
